...
- [x] ✅ 2026-06-11 (Quantum Sensor) / ✅ 2026-06-15 (Mythic items): All equipment effects wired. Quantum Sensor adds `boss_reward_boost` (+25% coin bonuses, higher drop chances, better rarity bias, 0.4 Epic chance on victory). Fixed EnsureStartingProfile loadout-reset bug. Void Engine adds `dash_kill_reset` (every enemy kill sets DashCooldownRemaining to 0). Storm Reactor adds `auto_charge_special` (TickAutoSpecial auto-fires Nova each tick the bar is full) and `double_nova` (TryActivateSpecial applies 2× damage via novaHits multiplier). 3 new EditMode tests (VoidEngine_DashKillReset, StormReactor_DoubleNova, StormReactor_AutoChargeSpecial).
